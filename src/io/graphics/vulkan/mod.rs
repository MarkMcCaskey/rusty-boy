...
use vulkano::framebuffer::{EmptySinglePassRenderPassDesc, RenderPass, AttachmentsList,
                           RenderPassAbstract};
...
pub struct VulkanRenderer {
    ...
    framebuffers: Option<Vec<Arc<Framebuffer<Arc<RenderPassAbstract + Send + Sync>,
                                             ((), Arc<SwapchainImage>)>>>>,
...
impl VulkanRenderer {
    pub fn new(app_settings: &ApplicationSettings) -> Result<Self, String> {
        ...
        debug!("Using device: {} (type: {:?})",
               physical.name(),
               physical.ty());

        let events_loop = winit::EventsLoop::new();
        let window = match winit::WindowBuilder::new()
    ...
    fn draw_gameboy(&mut self, gameboy: &Cpu, app_settings: &ApplicationSettings) {
        ...
        loop {
            ...
            if self.framebuffers.is_none() {
                let new_framebuffers = Some(self.images
                                                .iter()
                                                .map(|image| {
